...
function RightSideBlurLock({ children, text }: { children: ReactNode; text: string }) {
  return (
...
function UpgradeBanner({
  ...
  const title = lang === "en" ? "Unlock PRO" : "Atbloķē PRO";
  const points =
    lang === "en"
      ? ["Unlimited generations", "Copy with 1 click", "Editing + Manual builder"]
      : ["Neierobežota ģenerācija", "Kopēšana ar 1 klikšķi", "Rediģēšana + Manual builder"];
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <div style={{ color: "#6b7280", fontWeight: 800, fontSize: 13 }}>
          {lang === "en"
            ? `FREE remaining today: ${remainingToday}/${dailyLimit}`
            : `FREE atlikušas šodien: ${remainingToday}/${dailyLimit}`}
        </div>
        <a
          href={ctaHref}
          style={{
        ...
        >
          {lang === "en" ? "Upgrade to PRO" : "Atbloķēt PRO"}
        </a>
      </div>
      ...
      <div style={{ marginTop: 10, color: "#6b7280", fontWeight: 800, fontSize: 12 }}>
        {lang === "en"
          ? "Activates instantly after payment. Cancel anytime."
          : "Aktivizējas uzreiz pēc apmaksas. Atcelt vari jebkurā brīdī."}
      </div>
    </div>
  );
}
...
export default function AiPage() {
  ...
  const overlayText = "FREE režīmā kopēšana un rediģēšana ir bloķēta. Atbloķē PRO, lai vari kopēt un pielāgot.";

  // FREE → PRO MVP limits (localStorage)
  // Mērķis: iedot "wow" ar dažām ģenerācijām, bet atstāt motivāciju atbloķēt kopēšanu/Manual.
  const dailyLimit = 3;
  const demoCount = useMemo(() => (typeof window === "undefined" ? 0 : readDemoCount()), [generated, plan]);
  const remainingToday = Math.max(0, dailyLimit - demoCount);
  const limitMsg =
    lang === "en"
      ? `Daily FREE limit (${dailyLimit}/${dailyLimit}) reached. Unlock PRO for unlimited generations + copy + editing.`
      : `Šodienas FREE limits (${dailyLimit}/${dailyLimit}) ir sasniegts. Atbloķē PRO: neierobežota ģenerācija + kopēšana + rediģēšana.`;
  ...
  const pageInner: CSSProperties = { width: "100%", maxWidth: 980 };
  ...
  const adGroups = useMemo(() => {
    ...
    return groups.map((kws, i) => {
      ...
      const lines = kws.reduce<string[]>((acc, kw) => acc.concat(keywordLines(kw)), []);
        ...
        {showTop && (
          <>
            ...
            <div style={{ marginTop: 16, display: "flex", gap: 12, alignItems: "center" }}>
              ...
              >
                {lang === "en" ? "Unlock PRO" : "Atbloķēt PRO"}
              </a>
            ...
            {plan === "FREE" && (
              <div style={{ marginTop: 12 }}>
                <UpgradeBanner lang={lang} remainingToday={remainingToday} dailyLimit={dailyLimit} ctaHref={pricingPath} />
              </div>
            )}
                ...
                <RightSideBlurLock text={overlayText}>
                  <GoogleLikeAdCard
                    ...
                    onCopyBlock={() =>
                      setError(
                        lang === "en"
                          ? "Copy is locked in FREE. Unlock PRO to copy + edit."
                          : "Kopēšana FREE režīmā ir bloķēta. Atbloķē PRO, lai vari kopēt un rediģēt."
                      )
                    }
                  />
                </RightSideBlurLock>
