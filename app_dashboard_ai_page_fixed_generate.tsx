...
export default function AiPage() {
  ...
  const dailyLimit = 8;
  const limitMsg = `Šodienas FREE limits (${dailyLimit} ģenerācijas) ir sasniegts. Izveido kampaņu.`;
  ...
  async function onGenerate() {
    ...
    if (plan === "FREE") {
      ...
      if (current >= dailyLimit) {
        setError(limitMsg);
        setPaywall({
          title: langNow === "en" ? "Atbloķē FREE" : "Atbloķē FREE",
          text:
            langNow === "en"
              ? "Tas noņem dienas limitu un atbloķē kopēšanu + rediģēšanu."
              : "Tas noņem dienas limitu un atbloķē kopēšanu + rediģēšanu.",
          ctaLabel: langNow === "en" ? "Atbloķēt FREE" : "Atbloķēt FREE",
          ctaHref: pricingPath,
    ...
    setLoading(true);
    try {
            const r = await fetch("/api/ai/fetch-url", {
        method: "POST",
      ...
      try {
                const lr = await fetch("/api/ai/fetch-links", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ url: v }),
          cache: "no-store",
      ...
      for (const theme of chosenThemes) {
        for (let i = 0; i < 4; i++) {
          const ad = makeAdPreview({ theme, domain, goal, finalUrl: v, lang: langNow });
      ...
      setError("Kaut kas nogāja greizi. Pamēģini vēlreiz vai citu URL.");
    }
    finally {
      setLoading(false);
    }
  ...
  function onNewSample() {setGenerated(null);
    ...
    if (plan === "FREE") {
      ...
      if (current >= dailyLimit) {
        ...
        setPaywall({
          title: "Atbloķē FREE",
          text: "Tas noņem dienas limitu un atbloķē kopēšanu + rediģēšanu.",
          ctaLabel: "Atbloķēt FREE",
          ctaHref: pricingPath,
  ...
  function openUpgradePaywall(reason?: string) {
    ...
    setError(lang === "en" ? "This feature is locked on FREE." : "Šī funkcija FREE režīmā ir bloķēta.");
    setPaywall({
      title: lang === "en" ? "Unlock copy + editing" : "Atbloķē kopēšanu + rediģēšanu",
      text:
        lang === "en"
          ? "Upgrade to PRO to copy, edit, and export your campaign."
          : "Atbloķē pilnu piekļuvi, lai kopētu, rediģētu un eksportētu kampaņu.",
      ctaLabel: lang === "en" ? "Atbloķēt FREE" : "Atbloķēt pilnu piekļuvi",
      ctaHref: pricingPath,
          ...
          <>
            ...
            {isGenBlocked && (
              <div style={{ marginTop: 10, color: "#111827", fontWeight: 800, fontSize: 13 }}>
                🔓 Atbloķē FREE, lai turpinātu ģenerēt bez limita.
              </div>
            )}
            ...
            <div style={{ marginTop: 12, borderRadius: 16, background: "#f8fafc", padding: 12 }}>
              {plan === "FREE" && (
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 10 }}>
                  <button
                    type="button"
                    onClick={() => openUpgradePaywall("ad-copy")}
                    style={{
                      border: 0,
                      background: "#2563eb",
                      color: "white",
                  ...
                  >
                    ...
                    {lang === "en" ? "Copy ad" : "Kopēt reklāmu"}
                  </button>

                  <button
                    type="button"
                    onClick={() => openUpgradePaywall("ad-edit")}
                    style={{
                  ...
                  >
                    <span style={{ marginRight: 6 }}>🔒</span>
                    {lang === "en" ? "Edit text" : "Rediģēt tekstus"}
                  </button>

                  <button
                    type="button"
                    onClick={() => openUpgradePaywall("ad-export")}
                    style={{
                      border: 0,
                      background: "#111827",
                      color: "white",
...
{plan === "FREE" && (
  ...
  >
    ...
    <div style={{ marginTop: 6, color: "#374151", fontWeight: 700, lineHeight: 1.35 }}>
      Atbloķē FREE, lai varētu kopēt, rediģēt un eksportēt kampaņas struktūru.
    </div>
    <button
      type="button"
      onClick={() => {
        if (plan === "FREE" && remainingToday > 0) {
          window.location.href = pricingPath;
          return;
        }
        openUpgradePaywall("result-cta");
      }}
      style={{
        marginTop: 10,
        border: 0,
