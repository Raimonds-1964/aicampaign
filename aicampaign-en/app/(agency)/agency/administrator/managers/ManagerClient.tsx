...
function openManagerView(managerId: string) {
  const target = `/agency/manager/campaigns?managerId=${encodeURIComponent(managerId)}`;
  // ✅ Open a real route (not about:blank) that immediately redirects to target
...
export default function ManagerClient() {
  ...
  const freeCampaignsCount = useMemo(() => adminSelectors.freeCampaigns(s).length, [s]);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6">
      ...
      <div className={card}>
        <div className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div className="mt-1 text-2xl font-semibold text-white">Administrator</div>
            <div className="mt-2 text-white/70">
              Unassigned campaigns:{" "}
              <span className="font-semibold text-white">{freeCampaignsCount}</span>
            </div>
          </div>
      ...
      <div className={card}>
        <div className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="text-lg font-semibold text-white">Add a new manager</div>
            <div className="mt-1 text-white/55">Enter a name and click “Add”.</div>
          </div>
      ...
      <div className={card}>
          ...
          <table className="min-w-[900px] w-full text-left text-sm">
            <thead className="text-white/60">
              <tr className="border-b border-white/10">
                ...
                <th className="px-6 py-4">Campaign count</th>
                ...
                <tr key={m.id} className="border-b border-white/5 last:border-b-0">
                  ...
                  <td className="px-6 py-5">
                    <div className="flex justify-end gap-2">
                      ...
                      <button className={btnDanger} type="button" onClick={() => adminActions.deleteManager(m.id)}>
                        Delete
                      </button>
                    </div>
                  </td>
        ...
        <div className="px-6 py-3 text-xs text-white/45">
          “Delete” moves all campaigns from that manager back to the Administrator (Admin pool).
        </div>
      </div>
            ...
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              <div>
                ...
                <div className="text-sm text-white/60">
                  Opens the manager campaign view in a new window.
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                >
                  ...
                  <div className="text-xs text-white/50">
                    Opens the manager dashboard in admin view
                  </div>
                </button>
