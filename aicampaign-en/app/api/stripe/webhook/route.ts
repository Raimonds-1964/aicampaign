...
async function upsertSubscription(args: {
  ...
  // Look up the user by email (guaranteed by upsertUserByEmail)
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        const userPlan = toUserPlan(session.metadata?.userPlan ?? session.metadata?.planKey);
        ...
        // If this was a subscription checkout, session.subscription will contain the subscription ID.
        const subId = typeof session.subscription === "string" ? session.subscription : null;

        if (subId) {
          // TS-safe: Stripe SDK types can vary; avoid typing conflicts around current_period_end.
          const sub = (await stripe.subscriptions.retrieve(subId)) as any;

          const stripePriceId = sub?.items?.data?.[0]?.price?.id || sub?.plan?.id || ""; // legacy fallback
      ...
      case "customer.subscription.deleted": {
        ...
        // Look up the user by stripeCustomerId (unique in the User model)
        const user = await prisma.user.findUnique({
        ...
        const stripePriceId = sub?.items?.data?.[0]?.price?.id || sub?.plan?.id || "";
