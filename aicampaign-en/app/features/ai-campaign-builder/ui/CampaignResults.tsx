  ...
  return (
    <div className={card}>
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <div className="text-xs text-white/50">Max {maxLen} characters</div>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-white/5">
            <tr>
              ...
              <th className={`px-3 py-2 text-left ${th}`}>Text</th>
              <th className={`w-56 px-3 py-2 text-right ${th}`}>Actions</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
                ...
                <td className="px-3 py-2">
                  <div className="flex justify-end gap-2">
                    ...
                    <button className={btn} type="button" onClick={() => onToggle(i)}>
                      {toggleLabel ? toggleLabel(i) : r.included ? "Exclude" : "Include"}
                    </button>
                  </div>
                </td>
              </tr>
      ...
      <div className="overflow-hidden rounded-xl border border-white/10">
        <table className="w-full">
          <thead className="bg-white/5">
            <tr>
              <th className={`w-12 px-3 py-2 text-left ${th}`}>#</th>
              <th className={`px-3 py-2 text-left ${th}`}>Keyword</th>
              <th className={`w-56 px-3 py-2 text-right ${th}`}>Actions (include)</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
...
export default function CampaignResults({ draft }: Props) {
  ...
  const includedSitelinks = sitelinks
    .filter((x) => x.included)
  ...
  function guardMinRemove(section: "h" | "d") {
    if (section === "h" && headlinesIncludedCount <= MIN_HEADLINES) {
      alert(`You must keep at least ${MIN_HEADLINES} headlines.`);
      return false;
    }
    if (section === "d" && descIncludedCount <= MIN_DESCRIPTIONS) {
      alert(`You must keep at least ${MIN_DESCRIPTIONS} descriptions.`);
      return false;
    }
    return true;
  }
  ...
  function editRow(section: "h" | "d", idx: number) {
    ...
    const next = window.prompt(`Edit text (max ${max}):`, cur);
    if (next == null) return;

    const raw = String(next).trim();
    const final = raw.length > max ? smartTrim(raw, max) : raw;
    if (raw.length > max) alert(`That text was too long. A shortened version was applied (≤${max}).`);
  ...
  function editKeyword(which: "k" | "n", idx: number) {
    ...
    const next = window.prompt("Edit keyword:", cur);
    if (next == null) return;
  ...
  function addKeyword(which: "k" | "n") {
    const next = window.prompt(which === "k" ? "Add a keyword:" : "Add a negative keyword:", "");
    if (next == null) return;
    const val = String(next).trim();
    if (!val) return;
  ...
  function editSitelink(idx: number) {
    ...
    const nextTitle = window.prompt(`Edit sitelink text (max ${MAX_SITELINK}):`, cur.title);
    ...
    if (rawTitle.length > MAX_SITELINK) {
      alert(`That sitelink text was too long. A shortened version was applied (≤${MAX_SITELINK}).`);
    }

    const nextUrl = window.prompt("Edit sitelink URL:", cur.url);
    if (nextUrl == null) return;

    setSitelinks((p) =>
      p.map((r, i) => (i === idx ? { ...r, title, url: String(nextUrl).trim() } : r))
    );
  }
  ...
  function ensureMinOrAlert() {
    if (headlinesIncludedCount < MIN_HEADLINES) {
      alert(`Not enough headlines — you need at least ${MIN_HEADLINES}.`);
      return false;
    }
    if (descIncludedCount < MIN_DESCRIPTIONS) {
      alert(`Not enough descriptions — you need at least ${MIN_DESCRIPTIONS}.`);
      return false;
    }
    return true;
  }
  ...
  async function exportPdf() {
    ...
    const payload = {
      ...
      selected: {
        headlines: includedHeadlines
          .map((t) => (t.length > MAX_HEADLINE ? smartTrim(t, MAX_HEADLINE) : t))
          .slice(0, 6),
        descriptions: includedDescriptions
          .map((t) => (t.length > MAX_DESC ? smartTrim(t, MAX_DESC) : t))
          .slice(0, 4),
    ...
    if (!res.ok) {
      ...
      alert("PDF error: " + txt.slice(0, 800));
      return;
    }
...
Industry: ${draft.input.industry ?? ""}<br/>
Goal: ${draft.input.goal ?? ""} · Location: ${draft.input.location ?? ""} · Language: ${
      draft.input.language ?? ""
    } · Daily budget: ${draft.input.dailyBudget ?? ""} USD
</div>

<h2>Headlines (≤30)</h2>
<ul>${includedHeadlines.map((t) => `<li>${t}</li>`).join("")}</ul>

<h2>Descriptions (≤90)</h2>
...
<h2>Sitelinks (≤25)</h2>
<ul>${includedSitelinks
      .map((s) => `<li>${s.title}<br/><code>${s.url}</code></li>`)
      .join("")}</ul>
</body></html>`;
  ...
  function exportCsv() {
    ...
    includedNegativeKeywords.forEach((t, i) =>
      rows.push({ type: "negative_keyword", index: i + 1, text: t, url: "" })
    );
  ...
  function exportXls() {
    ...
    // Note: this is SpreadsheetML (.xls). The filename is intentionally kept as-is to avoid breaking expectations.
    downloadText("campaign-export.xls", toSpreadsheetML("Campaign Export", rows), "application/vnd.ms-excel");
  }

  function exportToAccount() {
    alert("Export to account: next step is wiring this up to the account store (Agency) or the Google Ads API.");
  }
    ...
    <div className="mt-6 space-y-4">
      {!minOk ? (
        <div className="rounded-2xl border border-rose-400/30 bg-rose-500/10 p-4 text-sm text-rose-200">
          Minimum requirements: at least <b>{MIN_HEADLINES}</b> headlines and at least{" "}
          <b>{MIN_DESCRIPTIONS}</b> descriptions.
        </div>
      ) : null}
      ...
      <div className={card}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-[280px]">
            ...
            <div className="mt-2 text-sm text-white/60">Campaign name</div>
            <input
            ...
            <div className="mt-2 text-sm text-white/60">
              URL: {draft.input.uri} · Industry: {draft.input.industry ?? "—"} · Location:{" "}
              {draft.input.location ?? "—"} · Language: {draft.input.language ?? "—"} · Daily budget:{" "}
              {draft.input.dailyBudget ?? "—"} USD
            </div>
          </div>
      ...
      <KeywordTable
        ...
        rows={keywords}
        onAdd={() => addKeyword("k")}
        onEdit={(i) => editKeyword("k", i)}
        onToggle={(i) => toggleKeyword("k", i)}
      />
      <KeywordTable
        title="Negative keywords"
        rows={negativeKeywords}
        onAdd={() => addKeyword("n")}
        onEdit={(i) => editKeyword("n", i)}
        onToggle={(i) => toggleKeyword("n", i)}
      />

      <div className={card}>
        <div className="mb-3 flex items-center justify-between gap-3">
          <div className="text-sm font-semibold text-white/80">Sitelinks (4)</div>
          <div className="text-xs text-white/50">Sitelink text max {MAX_SITELINK} characters</div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm text-white/60">
            Export includes <b>only</b> the selected text and inputs.
          </div>
