...
export default function KeywordsTable({ rows }: { rows: KeywordRow[] }) {
   ...
   * The server may render one snapshot of `rows`, but the browser can immediately
   * re-render with a different snapshot after store hydration.
   * To avoid mismatch warnings, we only render the table after mount (client-side).
   */
        ...
        <table className="min-w-[720px] w-full text-left text-sm">
          <thead className="text-white/60">
            <tr className="border-b border-white/10">
              ...
              <th className="px-4 py-3">1st Page</th>
          ...
          <tbody className="text-white/80">
            {safeRows.map((r, i) => (
              <tr
                // Prefer stable keys over `key={i}` to reduce incorrect diffing
                key={(r as any)?.id ?? (r as any)?.keyword ?? i}
                className="border-b border-white/5 last:border-b-0"
              >
                <td className="px-4 py-3">{(r as any).keyword}</td>
