...
function safeLoad(): AdminState | null {
  ...
  try {
    ...
    // Basic sanity: prevent empty/invalid shapes from breaking the UI.
    if (!Array.isArray(parsed.managers) || !Array.isArray(parsed.accounts))
      return null;
...
export const adminActions = {
  ...
  /** Unassign from manager -> account returns to the System Administrator */
  ...
  /** Delete a manager -> all of their accounts are reassigned to the System Administrator */
  ...
  addManager(name: string) {
    const id = `mgr-${Math.random().toString(16).slice(2)}`;
