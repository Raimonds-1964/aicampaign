...
export default function Client() {
  ...
  function openDetails(accountId: string, campaignId: string) {
    router.push(
      `/basic/administrator/accounts/${encodeURIComponent(accountId)}/campaigns/${encodeURIComponent(
        campaignId
      )}`
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-6">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-white/90">Campaigns</h1>
          {accountIdFilter && (
            <div className="mt-2 text-sm font-semibold text-white/60">
              Filter: account{" "}
              <span className="font-mono text-white/80">{accountIdFilter}</span>{" "}
              <Link
                className="ml-2 underline decoration-white/30 underline-offset-4 hover:decoration-white/60"
                href="/basic/administrator/campaigns"
              >
                Clear filter
              </Link>
            </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.04)] backdrop-blur">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="text-x2 font-extrabold text-white/90">Campaign overview</div>
      ...
      <div className="space-y-8">
        {filteredAccounts.map((acc) => {
          ...
          return (
            <div
              key={acc.id}
              className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.04)] backdrop-blur"
            >
              ...
              <div className="mt-6 space-y-6">
                {cs.length === 0 ? (
                  ...
                  cs.map((c) => {
                    ...
                    const spent =
                      typeof c.spentToday === "number" && Number.isFinite(c.spentToday)
                        ? c.spentToday
                        : 0;
                    ...
                    return (
                      <div key={c.id} className="space-y-3">
                        <div className="flex items-start justify-between gap-4">
                          <div className="min-w-0">
                            <div className="truncate text-2xl font-extrabold text-white/90">
                              {c.name}
                            </div>
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-3 text-sm font-semibold text-white/60">
                              ...
                              <span
                                className={`ml-2 inline-flex items-center rounded-full border px-3 py-1 text-xs font-extrabold ${badgeTone(
                                  health
                                )}`}
                              >
                                {badgeLabel(health)}
                        ...
                        <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
                          ...
                          <div className="mt-4 flex flex-wrap items-center gap-3">
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (confirm("Delete this campaign?")) deleteCampaign(String(c.id));
                              }}
                            >
                              Delete
                            </button>
