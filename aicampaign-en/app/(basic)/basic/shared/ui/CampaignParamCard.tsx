...
/** PRO namespace — so it never collides with Agency */
...
export default function CampaignParamCard(props: {
  ...
  status: Status;
  summary: string;
  /** Kept in props (can show in modal as "AI rationale") */
  aiSuggestion: string;
  ...
  // Sync across tabs/logs (PRO namespace)
  useEffect(() => {
    ...
    window.addEventListener("pro:ai-toggle", onCustom as EventListener);

    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener("pro:ai-toggle", onCustom as EventListener);
      try {
        bc?.close();
      } catch {
        // ignore
  ...
  const changes: AiChange[] = useMemo(() => {
    ...
    return [
      ...
      {
        id: "chg-3",
        title: "Update an RSA headline (asset)",
  ...
  return (
    <>
      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
        {/* ✅ Unified layout: actions always at the bottom (not on the right) */}
        <div className="min-w-0">
          ...
          {/* ✅ Requirement: if AI is off, show nothing under the header */}
          {aiEnabled ? (
            <>
              <div className="mt-2 text-sm text-white/70">{summary}</div>

              {/* ❌ Removed "AI suggestion" block from the card */}

              {/* ✅ CTA buttons at the bottom (consistent across cards) */}
              <div className="mt-4 flex flex-wrap gap-2">
                <a
                  ...
                  title={googleAdsDisabled ? "Demo mode: link disabled (no API)" : "Open Google Ads"}
                >
                  Open Google Ads ↗
                </a>
                ...
                <button
                  type="button"
                  onClick={openAiOffer}
                  className="rounded-lg bg-white/90 px-4 py-2 text-sm text-black hover:opacity-90"
        ...
        <div className="fixed inset-0 z-[80]">
          ...
          >
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              <div>
                <div className="text-base font-semibold text-white/90">AI suggestions</div>
            ...
            <div className="max-h-[70vh] overflow-auto p-4">
              <div className="mb-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <div className="mt-3 rounded-2xl border border-white/10 bg-black/20 p-3">
                  <div className="text-xs font-semibold text-white/60">AI rationale</div>
                  <div className="mt-1 text-sm text-white/80">{aiSuggestion}</div>
                </div>
              </div>

              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="text-sm font-semibold text-white/90">Recommended changes</div>
                <div className="mt-2 text-sm text-white/60">
                  Select what to apply. AI will only apply changes you approve.
                </div>
