...
export default function AiPage() {
  ...
  const overlayText = "Izveido kampaņu, lai atbloķētu un rediģētu paraugu.";

  const dailyLimit = 30;
  const limitMsg =
    lang === "en"
      ? `Today's FREE limit (${dailyLimit} generations) is reached. PRO unlocks unlimited generations + copy + editing.`
      : `Šodienas FREE limits (${dailyLimit} ģenerācijas) ir sasniegts. PRO atbloķē neierobežotu ģenerāciju + kopēšanu + rediģēšanu.`;
  ...
  return (
    <div style={pageShell}>
      <main style={{ ...pageInner, fontFamily: "Arial, sans-serif" }}>
        ...
        {showTop && (
          <>
            ...
            <div style={{ marginTop: 18, display: "flex", gap: 10, alignItems: "center" }}>
              ...
              <button
                onClick={onGenerate}
                disabled={!urlOk || loading || (plan === "FREE" && demoCount >= dailyLimit)}
                style={{
                  padding: "12px 18px",
                  borderRadius: 14,
                  border: 0,
                  background: !urlOk || loading || (plan === "FREE" && demoCount >= dailyLimit) ? "#93c5fd" : "#2563eb",
                  color: "white",
                  fontWeight: 900,
                  cursor: !urlOk || loading || (plan === "FREE" && demoCount >= dailyLimit) ? "not-allowed" : "pointer",
                  fontSize: 16,
                  minWidth: 240,
                }}
              >
        ...
        {plan === "FREE" && (
          <div style={{ marginTop: 10, fontWeight: 800, color: demoCount + 1 >= dailyLimit ? "#dc2626" : "#111827" }}>
            {lang === "en"
              ? `FREE: ${Math.max(0, dailyLimit - demoCount)}/${dailyLimit} generations left today`
              : `FREE: ${Math.max(0, dailyLimit - demoCount)}/${dailyLimit} ģenerācijas palikušas šodien`}
          </div>
        )}
        ...
        {plan === "FREE" && limitHit && (
          ...
          >
            <div style={{ fontWeight: 900 }}>
              {lang === "en"
                ? "Unlock PRO to keep generating + copy & edit"
                : "Atbloķē PRO: turpini ģenerēt + kopē un rediģē"}
            </div>
            <a
              href={pricingPath}
              style={{
              ...
              {plan === "PRO" && modelToShowSafe ? (
                ...
                <RightSideBlurLock text={overlayText}>
                  <GoogleLikeAdCard
                    ...
                    setError(
                      lang === "en"
                        ? "Copy is a PRO feature. Upgrade to unlock copying + editing."
                        : "Kopēšana ir PRO funkcija. Atbloķē PRO, lai kopētu un rediģētu."
                    )
                  }
                  />
                </RightSideBlurLock>
