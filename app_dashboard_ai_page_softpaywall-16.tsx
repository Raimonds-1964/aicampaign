...
export default function AiPage() {
  ...
  function openUnlockPaywall(opts?: { title?: string; text?: string; note?: string }) {
    // Vienots soft-paywall (FREE) — bez "PRO" putras tekstos
    const title = opts?.title ?? (lang === "en" ? "Unlock full access" : "Atbloķē FREE");
    const text =
      opts?.text ??
      (lang === "en"
        ? "Removes the daily limit and unlocks copy + editing."
        : "Tas noņem dienas limitu un atbloķē kopēšanu + rediģēšanu.");
    const note = opts?.note ?? (lang === "en" ? "Takes less than a minute." : "Aizņem mazāk nekā minūti.");
  ...
  function requireUnlock(message: string, opts?: { title?: string; text?: string; note?: string }) {
    setError(message);
    openUnlockPaywall(opts);
  }
  ...
  async function handleCopy(label: string, text: string) {
    if (plan === "FREE") {
      requireUnlock(lang === "en" ? "Copy is locked on FREE." : "Kopēšana FREE režīmā ir bloķēta.");
      return;
    }
              ...
              {plan === "PRO" && modelToShowSafe ? (
                ...
                <RightSideBlurLock text={overlayText}>
                  <GoogleLikeAdCard
                    ...
                    onCopyBlock={() => {
                      requireUnlock(
                        lang === "en" ? "Copy is locked on FREE." : "Kopēšana FREE režīmā ir bloķēta.",
                    ...
                    <div style={{ display: "grid", gap: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 4 }}>
                        ...
                        <button
                          onClick={() => {
                            if (plan === "FREE") {
                              requireUnlock(lang === "en" ? "Export is locked on FREE." : "Eksports FREE režīmā ir bloķēts.");
                              return;
                            }
                            downloadTextFile("campaign-export.csv", csvFull);
                          }}
                          style={{
                            border: "1px solid #d1d5db",
                            background: "white",
