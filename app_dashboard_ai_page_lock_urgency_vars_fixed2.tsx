...
const DAILY_LIMIT = 3; // FREE dienas limits
...
export default function AiPage() {
  ...
  const copyLabel = (en: string, lv: string) => (isFree ? "🔒 " : "") + (lang === "en" ? en : lv);
  ...
  const limitMsg = `Šodienas limits (${DAILY_LIMIT} ģenerācijas) ir sasniegts. Izveido kampaņu.`;


  const remainingToday = useMemo(() => {
    if (plan !== "FREE") return DAILY_LIMIT;
    const current = Number.isFinite(demoCount) ? demoCount : 0;
    return Math.max(0, DAILY_LIMIT - current);
  }, [plan, demoCount]);


  const lockPrefix = plan === "FREE" ? "🔒 " : "";
  const copyKeywordsLabel = lockPrefix + (lang === "en" ? "Copy keywords" : "Kopēt atslēgvārdus");

  const urgencyText =
    plan !== "FREE"
      ? ""
      : remainingToday === 1
      ? (lang === "en" ? " — Last one today" : " — Pēdējā ģenerācija šodien")
      : remainingToday === 0
      ? (lang === "en" ? " — Limit reached today" : " — Limits sasniegts šodien")
      : "";
          ...
          <>
            ...
            {plan === "FREE" && (
              <div style={{ marginTop: 14, fontSize: 13, fontWeight: 700, color: remainingToday <= 0 ? "#dc2626" : "#6b7280" }}>
                FREE: {remainingToday} / {DAILY_LIMIT} ģenerācijas palikušas šodien{urgencyText}
              </div>
            )}
                  ...
                  <div style={{ display: "grid", gap: 12 }}>
                    ...
                    <div style={{ display: "grid", gap: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 4 }}>
                        ...
                        >
                          {copyLabel("Copy CSV (Keywords)", "Kopēt CSV (atslēgvārdi)")}
                        </button>
                        ...
                        >
                          {copyLabel("Copy CSV (Ads + Sitelinks)", "Kopēt CSV (teksti + sitelinki)")}
                        </button>
                        ...
                        >
                          {copyLabel("Download CSV (Full)", "Lejupielādēt CSV (viss)")}
                        </button>
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 2 }}>
                      ...
                      >
                        {copyLabel("Copy full campaign", "Kopēt visu kampaņu")}
                      </button>
                      ...
                      >
                        {copyLabel("Copy ad texts", "Kopēt reklāmas tekstus")}
                      </button>
                    </div>
                  </div>
                </div>
