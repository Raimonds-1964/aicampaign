...
function looksLikeNoise(line: string) {
  ...
  const nonLetters = line.replace(/[a-zāčēģīķļņōŗšūž0-9\s.,:;!?'"“”()\-/]/gi, "");
...
function removeLikelyChrome($: cheerio.CheerioAPI) {
  ...
  const badRe =
    /(cookie|consent|banner|modal|popup|overlay|drawer|cart|basket|checkout|filter|filters|sort|login|register|account|profile|language|lang|newsletter|subscribe|gdpr|privacy|terms|breadcrumb|breadcrumbs|pagination|pager)/i;
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // 2) ja header nav, mēģina nolasīt meta (ar ātru utf-8 decode)
    ...
    // 3) decode ar atrasto charset
    let html = charset ? iconv.decode(buf, charset) : htmlUtf8;

    // fallback LV: windows-1257 (ja daudz “�”)
    const replacementCount = (html.match(/�/g) ?? []).length;
    if (replacementCount > 20 && !/1257|1252|latin|iso/i.test(charset ?? "")) {
      const alt = iconv.decode(buf, "windows-1257");
      const altBad = (alt.match(/�/g) ?? []).length;
      if (altBad < replacementCount) html = alt;
    }

    // ✅ Cheerio parse (bez decodeEntities opcijas — tā rada TS kļūdu tavā versijā)
