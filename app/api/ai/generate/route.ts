...
function titleCaseLv(s: string) {
  // vienkāršs: pirmā burta lielais
  const t = cleanLine(s);
  if (!t) return t;
...
export async function POST(req: Request) {
  try {
    ...
    // Stingrāks “schema” pieprasījums, lai modelis saprot, ka ad ir obligāts
    ...
    // 2) ja nepilns — retry 1x ar vēl stingrāku tekstu
    ...
    // 3) ja vēl joprojām nepilns — fallback, lai FREE vienmēr strādā
    if (!looksComplete(result)) {
