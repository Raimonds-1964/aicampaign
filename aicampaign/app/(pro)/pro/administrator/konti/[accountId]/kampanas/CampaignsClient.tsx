...
const card = "rounded-2xl border border-white/10 bg-white/5 backdrop-blur shadow-sm overflow-hidden";
const btn = "rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-medium text-white hover:bg-white/10";
...
function healthClass(h: Health) {
  ...
  if (h === "critical") return base + " border-red-400/30 text-red-200 bg-red-500/10";
  if (h === "warning") return base + " border-amber-400/30 text-amber-200 bg-amber-500/10";
...
function campaignGoogleAdsUrl(accountId: string, campaignId: string) {
  return `https://ads.google.com/aw/campaigns?account=${encodeURIComponent(accountId)}&campaignId=${encodeURIComponent(
    campaignId
  )}`;
}
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 text-sm text-white/70">
            Kampaņu skaits: <span className="font-semibold text-white/90">{items.length}</span>
          </div>
        </div>

        {/* ✅ Pro atpakaļ */}
        <Link className={btn} href={`/pro/administrator/konti`}>
          Atpakaļ
        </Link>
      </div>
              ...
              <div key={c.id} className="p-6">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-white/65">
                      <span>Statuss: {(c as any).status ?? "ACTIVE"}</span>
                      <span>·</span>
                      <span>ID: {c.id}</span>
                ...
                <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4">
                  ...
                  <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-white/10">
                    <div className={`h-full ${budgetBarClass(clamped)}`} style={{ width: `${clamped}%` }} />
                  </div>

                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    {/* ✅ Pro detaļas */}
                    <Link className={btn} href={`/pro/administrator/konti/${accountId}/kampanas/${c.id}`}>
                      Atvērt detaļas →
                    </Link>

                    {/* Demo režīms: var atstāt disabled, bet šobrīd atveram (kā anchor) */}
                    <a className={btn} href={campaignGoogleAdsUrl(accountId, c.id)} target="_blank" rel="noreferrer">
                      Atvērt Google Ads ↗
                    </a>
                  </div>
                </div>
