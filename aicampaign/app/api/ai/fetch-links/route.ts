...
function isBadSitelinkText(t: string) {
  ...
  if (!/[a-zāčēģīķļņōŗšūž]/i.test(t)) return true;
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // LV fallback: windows-1257
    const repl = (html.match(/�/g) ?? []).length;
    if (repl > 20 && !/1257|1252|latin|iso/i.test(charset ?? "")) {
      const alt = iconv.decode(buf, "windows-1257");
      if ((alt.match(/�/g) ?? []).length < repl) html = alt;
    }

    // ✅ Cheerio parse (bez decodeEntities opcijas)
