  ...
  try {
    ...
    if (!developerToken) {
      return NextResponse.json(
        { ok: false, error: "NO_DEVELOPER_TOKEN" },
        { status: 400 }
      );
    }
    ...
    if (!refreshToken) {
      return NextResponse.json(
        {
          ok: false,
          error: "NO_DEBUG_REFRESH_TOKEN",
          message: "Ieliec GOOGLE_DEBUG_REFRESH_TOKEN .env.local",
        },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: "SERVER_CRASH", message: String(e) },
      { status: 500 }
    );
  }
}
