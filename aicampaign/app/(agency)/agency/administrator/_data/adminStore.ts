...
export const adminActions = {
  /** Piešķirt kontu konkrētam managerim */
  assignAccountToManager(accountId: string, managerId: string) {
    setState({
      ...state,
      accounts: state.accounts.map((a) => (a.id === accountId ? { ...a, ownerId: managerId } : a)),
    });
  },

  /** Noņemt no managera -> konts pāriet pie Administrator */
  moveAccountToAdmin(accountId: string) {
    setState({
      ...state,
      accounts: state.accounts.map((a) => (a.id === accountId ? { ...a, ownerId: ADMIN_ID } : a)),
    });
  },

  /** Dzēst manageri -> visi viņa konti pāriet pie Administrator */
  deleteManager(managerId: string) {
    setState({
      managers: state.managers.filter((m) => m.id !== managerId),
      accounts: state.accounts.map((a) => (a.ownerId === managerId ? { ...a, ownerId: ADMIN_ID } : a)),
    });
  },

  /** (Demo) pievienot manageri */
