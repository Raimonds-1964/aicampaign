...
function googleAdsCampaignUrl(accountId: string, campaignId: string) {
  // Demo: īstais URL vēlāk, kad būs API. Šis der kā “dziļais links” testiem.
...
export default function Client() {
  ...
  const ownerNameById = useMemo(() => {
    ...
    map.set(ADMIN_OWNER_ID, "Administrator");
  ...
  const allCampaigns = useMemo(() => adminSelectors.campaigns(s), [s]);
  ...
  const accountHeader = useMemo(() => {
    if (accountFilter === "all") return { name: "Visi konti", id: "" };
    const acc = accounts.find((a) => a.id === accountFilter) ?? null;
    if (!acc) return { name: "Nezināms konts", id: accountFilter };
    return { name: acc.name, id: acc.id };
  }, [accounts, accountFilter]);

  function managerProfileHref(ownerId: string) {
    return `/agency/administrator/manager/${encodeURIComponent(ownerId)}`;
  }
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-2xl font-semibold text-white">Kampaņas</div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <div className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/85">
              <span className="text-white/60">Konts:</span>{" "}
              <span className="font-semibold text-white">{accountHeader.name}</span>{" "}
              {accountHeader.id ? (
                <span className="ml-2 font-mono text-xs text-white/45">{accountHeader.id}</span>
              ) : null}
            </div>
            ...
            <div className="min-w-[260px]">
              ...
              >
                <option value="all">Visi konti</option>
            ...
            {/* Brīvās kampaņas admin profilā */}
            <Link className={btn} href={managerProfileHref(ADMIN_OWNER_ID)}>
              Administrator profils (brīvās kampaņas) →
            </Link>
          </div>
        </div>

        {/* ===== Actions (bez “Skatīt kā” un bez kontu pievienošanas) ===== */}
        <div className="flex flex-wrap items-center gap-2">
          <button
            className={btn}
            type="button"
            disabled={accounts.length === 0 || accountFilter === "all"}
            onClick={() => {
              const id = accountFilter === "all" ? accounts[0]?.id : accountFilter;
              if (!id) return;
              adminActions.addCampaign(id, `Demo Campaign #${Math.floor(100 + Math.random() * 900)}`);
            }}
          >
                      + Demo kampaņa
          </button>
      ...
      {/* ===================== Visas kampaņas (filters) ===================== */}
      <div className={card}>
        <div className="border-b border-white/10 p-5">
          <div className="text-base font-semibold text-white">Visas kampaņas</div>
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-3">
            ...
            >
              <option value="all">Visi manager</option>
            ...
            <div className="text-sm text-white/50 md:justify-self-end md:self-center">
              Kopā: <span className="font-semibold text-white/80">{filteredCampaigns.length}</span>
            </div>
          </div>
        </div>
              ...
              {filteredCampaigns.map((c: any) => {
                ...
                return (
                  <tr key={c.id} className="border-b border-white/5 last:border-b-0">
                    ...
                    <td className="px-5 py-4">{managerCell}</td>

                    <td className="px-5 py-4">
                      {/* tikai krāsains statuss no detaļām */}
                      <span
                        className={
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-end gap-2">
                        <Link className={btn} href={`/agency/administrator/konti/${c.accountId}/kampanas/${c.id}`}>
                          Detaļas →
                        </Link>
        ...
        <div className="px-5 py-3 text-xs text-white/45">
          Brīvās kampaņas ir Administrator profilā. “Skatīt kā” tagad ir Manager sadaļā.
        </div>
      </div>
    </div>
  );
