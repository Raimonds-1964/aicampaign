...
function openManagerView(managerId: string) {
  const url = `/agency/manager/konti?managerId=${encodeURIComponent(managerId)}`;
...
export default function Client() {
  ...
  const filtered = useMemo(() => {
    ...
    return managers.filter((m: Manager) => m.name.toLowerCase().includes(query) || m.id.toLowerCase().includes(query));
  }, [managers, q]);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="text-4xl font-semibold text-white">Manager</div>
          <div className="mt-2 text-white/60">
            Admin skats: pārvaldi managerus un atver viņu skatījumu (“Skatīt kā”).
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link className={btn} href="/agency/administrator/kampanas">
            Kampaņas →
          </Link>
        </div>
      </div>
      ...
      <div className={card}>
        <div className="border-b border-white/10 p-5">
          <div className="text-base font-semibold text-white">Manager saraksts</div>

          <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
            <input
              className={input}
              placeholder="Meklēt pēc vārda vai ID..."
              value={q}
                ...
                <tr key={m.id} className="border-b border-white/5 last:border-b-0">
                  ...
                  <td className="px-5 py-4">
                    <div className="flex justify-end gap-2">
                      ...
                      <Link className={btn} href={`/agency/administrator/manager/${encodeURIComponent(m.id)}`}>
                        Profils →
                      </Link>
                    </div>
                  </td>
        ...
        <div className="px-5 py-3 text-xs text-white/45">
          “Skatīt kā” atver manager paneli jaunā logā ar `?managerId=...`.
        </div>
      </div>
            ...
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              <div>
                ...
                <div className="text-sm text-white/60">
                  Atvērs manager paneli jaunā logā.
                </div>
              </div>
            ...
            <div className="max-h-[70vh] overflow-auto p-4">
              <div className="grid grid-cols-1 gap-2">
                ...
                >
                  ...
                  <div className="text-xs text-white/50">Admin skatījums manager panelī</div>
                </button>
