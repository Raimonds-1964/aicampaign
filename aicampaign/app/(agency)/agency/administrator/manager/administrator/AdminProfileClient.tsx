...
const th =
  "bg-white/5 text-left text-xs font-semibold text-white/60 px-5 py-3";
...
export default function AdminProfileClient() {
  ...
  const freeCampaigns = useMemo(
    () => adminSelectors.freeCampaigns(s) as any[],
    [s]
  );

  const managers: AdminManager[] = useMemo(
    () => adminSelectors.managers(s) as any,
    [s]
  );
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6">
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <Link href="/agency/administrator/manager" className={btn}>
          Atpakaļ
        </Link>
      </div>
      ...
      <div className={card}>
        <div className="border-b border-white/10 px-5 py-4">
          <div className="text-sm font-semibold text-white/90">
            Brīvās kampaņas (nav piešķirtas manager)
          </div>
          <div className="mt-1 text-xs text-white/50">
            Pie katras kampaņas izvēlies manager dropdownā — piešķiršana notiek uzreiz.
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[1000px] w-full">
            <thead>
              <tr>
                <th className={th}>Kampaņa</th>
                <th className={th + " text-center"}>Statuss</th>
                <th className={th + " text-center"}>Budžets</th>
                <th className={th}>Piešķirt manager</th>
                <th className={th + " text-right"}>Darbības</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={c.id} className="border-t border-white/10">
                    ...
                    <td className={tdCenter}>
                      <div
                        className={`mx-auto h-4 w-4 rounded-full ${statusColor(
                          c.health
                        )}`}
                      />
                    </td>
                    ...
                    <td className={tdCenter}>
                      <span
                        className={`rounded-full border px-3 py-1 text-xs font-semibold ${budgetColor(
                          percent
                        )}`}
                      >
                        {percent}%
                    ...
                    <td className={td}>
                      <select
                        ...
                        value={c.ownerId}
                        onChange={(e) =>
                          adminActions.assignCampaign(c.id, e.target.value)
                        }
                      >
        ...
        <div className="border-t border-white/10 px-5 py-3 text-xs text-white/50">
          Piešķirot kampaņu managerim, tā pazūd no Admin pool un parādās manager profilā.
        </div>
      </div>
    </div>
  );
