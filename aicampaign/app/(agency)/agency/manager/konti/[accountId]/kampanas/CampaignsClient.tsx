...
export default function CampaignsClient() {
  ...
  const managerId = sp.get("managerId") ?? ""; // manager skats tiek nests ar query
  ...
  const campaigns = useMemo(() => {
    ...
    // manager view: ja ir managerId, filtrējam pēc ownerId
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-1 text-4xl font-semibold text-white">Kampaņas</div>
          ...
          <table className="min-w-[900px] w-full text-left text-sm">
            ...
            <tbody className="text-white/85">
              {filtered.map((c: any) => {
                ...
                const detailsHref =
                  `/agency/manager/konti/${encodeURIComponent(accountId)}/kampanas/${encodeURIComponent(c.id)}` +
