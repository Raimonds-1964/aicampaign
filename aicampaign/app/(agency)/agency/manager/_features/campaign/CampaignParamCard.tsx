...
const label = (s: CheckStatus) => (s === "ok" ? "OK" : s === "warning" ? "Jāuzlabo" : "Kritisks");
...
export default function CampaignParamCard(props: {
  ...
  // auto-fix / task
  onAutoFix?: () => void;        // ja ir – izpilda automātisko
  onCreateTask?: () => void;     // ja nav auto-fix – izveido manuālu uzdevumu
  canAutoFix?: boolean;
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`inline-flex rounded-full border px-2 py-0.5 text-xs ${badge(status)}`}>{label(status)}</span>
          </div>
        ...
        <div className="flex shrink-0 flex-col gap-2">
          ...
          <button
            ...
            title={canAutoFix ? "Automātiski izpildīt" : "Izveidot manuālu uzdevumu"}
          >
            Izpildīt AI ieteikumu
          </button>
        </div>
      </div>
    </div>
