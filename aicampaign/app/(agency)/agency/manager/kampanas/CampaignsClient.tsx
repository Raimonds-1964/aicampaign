...
/** Statuss kā Admin: tikai krāsains aplītis (bez teksta) */
...
export default function CampaignsClient() {
  ...
  const managerId = sp.get("managerId") ?? "m1"; // default demo
  ...
  // Visas kampaņas no shared store
  const all = useMemo(() => managerSelectors.campaigns(s), [s]);

  // Rādām tikai konkrētā manager kampaņas
  const mine = useMemo(() => {
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <div className="mt-1 text-2xl font-semibold text-white">Kampaņas</div>
          <div className="mt-2 text-white/60">
            Manager skats: redzi tikai sev piešķirtās kampaņas
          </div>
        </div>

        <div className="flex flex-col items-end gap-2">
          <div className="text-sm text-white/60">Skaties kā manager</div>
          <select
            className="w-[260px] rounded-xl border border-white/10 bg-black px-3 py-2 text-sm text-white outline-none"
            value={managerId}
            onChange={(e) => {
              const next = e.target.value;
              const url = `/agency/manager/kampanas?managerId=${encodeURIComponent(next)}`;
          ...
          {!manager ? (
            <div className="text-sm text-white/45">Nav manager (demo dati nav ielādēti)</div>
          ) : (
            <div className="text-sm text-white/45">
              Skats: <span className="text-white/80">{manager.name}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className={card}>
        <div className="border-b border-white/10 p-5">
          <div className="text-base font-semibold text-white">Kampaņu saraksts</div>
          ...
          <table className="min-w-[900px] w-full text-left text-sm">
            ...
            <tbody className="text-white/85">
              {mine.map((c: any) => {
                ...
                const detailsHref = `/agency/manager/konti/${encodeURIComponent(accountId)}/kampanas/${encodeURIComponent(
                  c.id
                )}?managerId=${encodeURIComponent(managerId)}`;

                // ✅ poga vienmēr ir, tikai disabled, ja vēl nav linka
              ...
              {mine.length === 0 ? (
                <tr>
                  <td className="px-5 py-10 text-white/60" colSpan={4}>
                    Nav piešķirtu kampaņu šim manager.
                  </td>
                </tr>
              ) : null}
            </tbody>
        ...
        <div className="px-5 py-3 text-xs text-white/45">
          Šeit redzamas tikai kampaņas ar <span className="font-mono">ownerId = {managerId}</span>.
          Admin pool (admin) šeit netiek rādīts.
        </div>
      </div>
    </div>
  );
