...
export type Task = {
  ...
  // optional – lai varam krāsot/pāriet uz kampaņu utt.
  severity?: TaskSeverity;

  // te glabājam accountId/campaignId/checkKey/report u.c.
...
export function setTaskDone(id: string) {
  const items = load().map((t) => (t.id === id ? { ...t, status: "done" as const } : t));
  save(items);
}

export function setTaskOpen(id: string) {
  const items = load().map((t) => (t.id === id ? { ...t, status: "open" as const } : t));
  save(items);
}
