  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          <div className="text-sm font-semibold text-white/60">Konts</div>

          {/* ✅ konta nosaukums mazāks un pelēks */}
          ...
          <div className="mt-3 text-sm font-semibold text-white/50">
            Basic limits: maksimums {limits.maxCampaignsPerAccount} kampaņas
          </div>
        </div>

        <div className="flex items-center gap-2">
          <Link className={btn} href="/basic/administrator/konti">
            Konti →
          </Link>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-[0_0_0_1px_rgba(255,255,255,0.04)] backdrop-blur">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="text-xl font-extrabold text-white">Kampaņas</div>
          <div className="text-sm font-semibold text-white/60">
            Skaits: {filtered.length}
          </div>
        </div>
      ...
      <div className="space-y-6">
        {filtered.length === 0 ? (
          ...
          filtered.map((c) => {
            const health =
              typeof c.health === "number" && Number.isFinite(c.health)
                ? c.health
                : 50;
            const spent =
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-3 text-sm font-semibold text-white/60">
                        ...
                        <span>
                          ID:{" "}
                          <span className="font-mono text-white/80">{c.id}</span>
                        </span>
                  ...
                  <div className="mt-5 rounded-3xl border border-white/10 bg-black/30 p-5">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3">
                      <button
                        ...
                        onClick={() =>
                          router.push(
                            `/basic/administrator/konti/${encodeURIComponent(
                              accountId
                            )}/kampanas/${encodeURIComponent(String(c.id))}`
                          )
                        }
                      >
                        Atvērt detaļas →
