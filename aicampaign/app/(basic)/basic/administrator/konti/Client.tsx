    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 text-sm font-semibold text-white/60">
            Basic plānā var pieslēgt tikai vienu kontu.
          </div>
        </div>

        {/* ✅ tikai 1 poga: Pievienot AI kontu */}
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            className={btn}
            onClick={() => addAiAccount()}
            title="Izveidot AI demo kontu"
          >
            Pievienot AI kontu
          </button>
        </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur">
        <div className="grid grid-cols-12 gap-2 border-b border-white/10 px-5 py-3 text-xs font-semibold text-white/50">
          <div className="col-span-9">Konts</div>
          <div className="col-span-3 text-right">Darbības</div>
        </div>
        ...
        {!account ? (
          <div className="px-5 py-10">
            ...
            <div className="mt-2 text-sm font-semibold text-white/60">
              Spied “Pievienot AI kontu”, lai izveidotu demo kontu.
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-12 gap-2 px-5 py-5">
            ...
            {/* ✅ tikai 2 pogas: Skatīt Google Ads / Kampaņas */}
            <div className="col-span-3 flex flex-wrap justify-end gap-2">
              <a
                ...
                rel="noreferrer"
                className={btn}
                title="Skatīt Google Ads"
              >
                Skatīt Google Ads ↗
              </a>

              {/* ✅ FIX: Basic “īstā” kampaņu lapa ir /basic/administrator/kampanas */}
              <Link
                href={`/basic/administrator/kampanas?accountId=${encodeURIComponent(
                  account.id
                )}`}
                className={btn}
              >
      ...
      <div className="rounded-3xl border border-white/10 bg-white/5 p-5 text-sm font-semibold text-white/60">
        Basic ierobežojumi: maksimums <b className="text-white/80">1 konts</b>{" "}
        un <b className="text-white/80">5 kampaņas</b>. Pārskati nav pieejami.
      </div>
    </div>
  );
}
