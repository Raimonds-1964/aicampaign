...
export default function PricingPage() {
  ...
  // Upgrade reason banner (from FREE locks)
  useEffect(() => {
    try {
      const reason = localStorage.getItem("upgrade_reason") || "";
      const from = localStorage.getItem("upgrade_from") || "";

      const msgMap: Record<string, string> = {
        limit_reached: "Tu sasniedzi šodienas FREE limitu — atbloķē FREE, lai turpinātu.",
        url_locked: "Ģenerēšana šodien ir bloķēta — atbloķē FREE, lai turpinātu.",
        copy_locked: "Kopēšana FREE režīmā ir bloķēta — atbloķē FREE.",
        edit_locked: "Rokas rediģēšana FREE režīmā ir bloķēta — atbloķē FREE.",
        export_locked: "Eksports FREE režīmā ir bloķēts — atbloķē FREE.",
      };

      const msg = msgMap[reason] || "";
      if (msg) setUpgradeMsg(msg);

      // keep from for debugging if needed
      // console.log("upgrade_from:", from, "upgrade_reason:", reason);

      localStorage.removeItem("upgrade_reason");
      localStorage.removeItem("upgrade_from");
    } catch {}
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Vienkāršs ceļš uz kampaņu: izvēlies plānu, uzģenerē paraugu un turpini uz kampaņas izveidi.
          Mērķis: vairāk klientu un pārdošanas.
        </p>
